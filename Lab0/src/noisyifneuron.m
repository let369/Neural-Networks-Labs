...
i=1; %initialization of i that counts the legends 
     %that we are gonna have in our figure

for tau=tau_vector %sweep of tau value while keeping the 
		   %other 2 variables constant
	for n=2:nstep
	...
	figure(figHandleTau) %use of figure command so that our plots for
                  	     %the diffeerent sweeps be in different figures
	legendInfo{i} = ['Tau = ' num2str(tau) ... %add the values of
	' Mean Spike interval = ' num2str(mean(isi(tspike)))]; 
 	%the sweeping var as legend + the mean of interval spikes
	tspike = []; %initialization of tspike table for the next sweep
	hold all %hold all command so that all plots of this iteration 
		 %are represented at the same figure
...
for theta=theta_vector
	 ...
	 legendInfo{i} = ['Theta = ' num2str(theta) ...
	 ' Mean Spike interval = ' num2str(mean(isi(tspike)))];
	 tspike = [];
	 hold all
